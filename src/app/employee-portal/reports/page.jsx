...
const fallbackReportOptions = [
  { id: 'attendance', title: 'Attendance history', description: 'Daily presence, late marks, WFH logs.', icon: Clock, formats: ['CSV', 'PDF'] },
  { id: 'expenses', title: 'Expense submissions', description: 'All non-advance and advance-based claims.', icon: CreditCard, formats: ['CSV', 'XLSX'] },
  { id: 'requests', title: 'Leave & request log', description: 'Leaves, WFH, and support tickets filed.', icon: ClipboardList, formats: ['CSV'] },
];
...
export default function EmployeeReportsPage() {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/employee-portal/reports`, {
          headers: { ...(token ? { Authorization: `Bearer ${token}` } : {}) },
        });
        if (res.ok) {
          const json = await res.json();
          if (json?.success && json?.data?.reports) {
            setReportsData(json.data.reports);
          }
        }
      } catch (err) {
  ...
  const reportOptions = reportsData || fallbackReportOptions;
    ...
    <div className="space-y-6">
      ...
      <Card className="border-none bg-gradient-to-br from-amber-50 to-orange-100/60 shadow-lg shadow-amber-100/70">
        ...
        <CardContent className="text-sm text-amber-900/80">
          Report generation is instant for the past 12 months. Older data will be delivered to your inbox within one
          business day.
        </CardContent>
      </Card>
    </div>

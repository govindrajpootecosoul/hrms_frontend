...
export async function POST(request) {
  try {
    ...
    const tempDir = join(tmpdir(), `amazon-credit-note-${Date.now()}`);
    ...
    const scriptPath = join(process.cwd(), 'scripts', 'amazon_credit_note_extractor.py');
          ...
          try {
            ...
            resolve(new NextResponse(excelBuffer, {
              headers: {
                ...
                'Content-Disposition': `attachment; filename="amazon_credit_notes_${Date.now()}.xlsx"`,
              },
            }));
          } catch (fileError) {

...
export async function GET(request) {
  try {
    ...
    const response = await fetch(getAssetTrackerApiUrl(`/settings/categories${search}`), {
      method: 'GET',
      cache: 'no-store',
    });
    return proxyJsonResponse(response);
  } catch (error) {
    console.error('Error fetching category settings via backend:', error);
...
export async function PUT(request) {
  try {
    ...
    const response = await fetch(getAssetTrackerApiUrl('/settings/categories'), {
      method: 'PUT',
    ...
    console.error('Error saving category settings via backend:', error);

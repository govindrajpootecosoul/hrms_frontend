...
export async function POST(request) {
  try {
    ...
    const tempDir = join(tmpdir(), `gst-b2b-file-processing-${Date.now()}`);
    ...
    const scriptPath = join(process.cwd(), 'scripts', 'gst_b2b_file_processing.py');
    const outputPath = join(tempDir, `GST_B2B_Combined_${Date.now()}.xlsx`);
    ...
    return new Promise((resolve, reject) => {
      ...
      pythonProcess.on('close', async (code) => {
        try {
          if (code !== 0) {
            console.error('GST B2B File Processing stderr:', stderr);
            console.error('GST B2B File Processing stdout:', stdout);
            return reject(
              NextResponse.json(
          ...
          resolve(
            new NextResponse(outputBuffer, {
              headers: {
                ...
                'Content-Disposition': `attachment; filename="GST_B2B_Combined_${Date.now()}.xlsx"`,
              },
            })
          );
        } catch (error) {
    ...
    console.error('GST B2B File Processing API error:', error);
    return NextResponse.json(

...
export async function POST(request) {
  try {
    ...
    // Validate file types - only accept Excel and CSV files
    ...
    const tempDir = join(tmpdir(), `book-reconcile-${Date.now()}`);
    ...
    const scriptPath = join(process.cwd(), 'scripts', 'book_keeping_file_processing.py');
    const outputPath = join(tempDir, 'cleaned_book_keeping.csv');
    // If single file, use file path; if multiple files, use directory
    ...
    return new Promise((resolve, reject) => {
      ...
      pythonProcess.on('close', async (code) => {
        try {
          if (code !== 0) {
            console.error('Book Reconcile stderr:', stderr);
            console.error('Book Reconcile stdout:', stdout);
            return reject(
              NextResponse.json(
          ...
          resolve(
            new NextResponse(outputBuffer, {
              headers: {
                ...
                'Content-Disposition': `attachment; filename="cleaned_book_keeping_${Date.now()}.csv"`,
              },
            }),
          );
        } catch (error) {
    ...
    console.error('Book Reconcile API error:', error);
    return NextResponse.json(

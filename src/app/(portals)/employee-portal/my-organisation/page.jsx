...
export default function EmployeeOrganisationPage() {
  ...
  useEffect(() => {
    const fetchOrg = async () => {
      try {
        ...
        const res = await fetch(`${API_BASE_URL}/employee-portal/org`, {
  ...
  const myDepartment = useMemo(() => {
    ...
    const departments = orgData?.departments || mockData.orgChart.departments;
    return departments.find((dept) =>
      dept.name.toLowerCase().includes(employee.department.toLowerCase())
    );
  }, [user, orgData]);

  const otherDepartments = (orgData?.departments || mockData.orgChart.departments).filter((dept) => dept.id !== myDepartment?.id);
  ...
  const renderTree = (dept) => (
    <div className="mt-3 space-y-3 rounded-lg bg-white/70 p-4 text-sm shadow-inner">
      ...
      <div>
        ...
        <div className="mt-1 grid gap-2 md:grid-cols-2">
          {dept.seniorManagers?.map((leader) => (
      ...
      <div>
        ...
        {dept.leads && (
          <div className="mt-2 grid gap-2 md:grid-cols-3">
            {dept.leads.map((leader) => (
              <div
                key={leader.name}
                className="rounded-md border border-dashed border-slate-300 bg-slate-50/80 p-2 text-xs text-slate-600"
              >
          ...
          {[myDepartment, ...otherDepartments]
            ...
            .map((dept) => (
              <div key={dept.id} className="rounded-xl border border-slate-200 bg-white/90 p-4 shadow-sm">
                ...
                {expandedDept === dept.id && renderTree(dept)}
              </div>
            ))}
        </CardContent>
      </Card>

...
export default function EmployeeLayout({ children }: { children: React.ReactNode }) {
  ...
  const [redirectState, setRedirectState] = useState<'none' | 'login' | 'dashboard'>('none');
  ...
  if (redirectState === 'login') {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="rounded-2xl border bg-white px-8 py-6 text-center shadow-lg shadow-slate-200/70">
          <p className="text-base font-medium text-slate-900">Redirecting you to login…</p>
          <p className="mt-1 text-sm text-slate-500">
            Your session is not active. Continue to sign in and get back to your workspace.
          </p>
          <Button
            className="mt-4"
            onClick={() => {
  ...
  if (redirectState === 'dashboard') {
    return (
      <div className="flex min-h-screen items-center justify-center bg-slate-50">
        <div className="rounded-2xl border bg-white px-8 py-6 text-center shadow-lg shadow-slate-200/70">
          <p className="text-base font-medium text-slate-900">Redirecting you to the dashboard…</p>
          <p className="mt-1 text-sm text-slate-500">
            This space is tailored for employees. We’ll take you back to your main dashboard.
          </p>
          <Button
            className="mt-4"
            onClick={() => {
